...
public class StickxNLEpop11{
//   <title>Extended DNA Sticker

//globals needed for NLE
//note:  capital S3 and S5 to avoid naming conflict with Stickx
static boolean s0,s1,s2,S3,s4,S5,s6,s7,s8,s9,s10;
static boolean c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10;
static boolean z1,z2,z3,z4,z5,z6,z7,z8,z9;
static boolean f1,f2,f3,f4,f5,f6,f7,f8,f9,f10;
...
public static void gennN(int start)
{int nN,iI;
 nN = getnN(start);
...
public static void printnN(int start)
{int nN,iI;
 nN = getnN(start);
 ...
 {mywrite(start+" iI="+iI+" ");
  if (iI == 1)
    //combine(Mts+nN,nN+1);
    mywriteln("combine("+(0+Mts+nN)+","+(1+nN)+")");
  else
    //separate3(2+nN-iI,Mts+nN-iI+1,Mts+nN-iI+2,2+nN-iI);
    mywriteln("separate3("+(2+nN-iI)+","+
    (0+Mts+nN-iI+1)+","+(0+Mts+nN-iI+2)+","+(2+nN-iI)+")");
 }
}

public static void fetchpipe(int addr)
{int i;
 combine(Mts+aw-1,M);
 ...
 */
 ...
 //for (start=1015; start<=1015;start++)
 //for (start=2038; start<=2039;start++)
 {
  for(i=0;i<=M;i++)
  ...
  //if ((start == 59)||(start==123)) combine(8,2);
   // 2^6-5=59, 2^10-9=1015 (no 2^12-10=2038),  2^(aw-1)-aw+2
  if ((start&0x3f)==59)   combine(Mts+2-1, Mt+2); //aw>=7?
  if ((start&0x3ff)==1015) separate3/*(2,12,3,2); */ (Mt+2,Mts+2-1,Mt+2+1,2); //  aw>=11?
   //NO if ((start&0x3ff)==1015) combine(Mts+3-1, Mt+3); //aw>=11?
   //NO if ((start&0x3ff)==2038) combine(Mts+3-1, Mt+3); //aw>=12?
