...
static boolean s0,s1,s2,S3,s4,S5,s6,s7,s8;
static boolean c0,c1,c2,c3,c4,c5,c6,c7,c8;
static boolean z1,z2,z3,z4,z5,z6,z7;
static boolean f1,f2,f3,f4,f5,f6,f7,f8;
...
{ int i;
  ...
  if ((start&0x3f)==59)   combine(Mts+2-1, Mt+2); //aw>=7?
   // 2^6-5=59, 2^12-10=2038,  2^(aw-1)-aw+2
}
...
{int i,jj,iii,start;
 ...
 //for (start=249; start<=257;start++)
 {
  for(i=0;i<=M;i++)
    if (i != 2*Mts-1) combine(2*Mts-1, Mt+i);
  ...
  { //mywrite(form,"if (start=="+jj+") {");
    ...
    getCS(Mt,Mts);
    cstate = "C:"+c0+c1+c2+c3+c4+c5+c6+c7+c8+" S:"+s0+s1+s2+S3+s4+S5+s6+s7+s8;
    for(i=0;i<=M;i++)
    ...
    cteststate = "C:"+c0+c1+c2+c3+c4+c5+c6+c7+c8+" S:"+s0+s1+s2+S3+s4+S5+s6+s7+s8;
